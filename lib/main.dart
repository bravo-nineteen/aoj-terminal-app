                ...
                return Padding(
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Column(
                        children: [
                          _AOJDesktopIcon(icon: app.icon, accent: app.accent),
              ...
              children: [
                Expanded(
                  child: _InfoCard(
                    ...
                    accent: window.accent,
                    children: [
                      _InfoLine('State', systemStatus),
                      _InfoLine('Events', appState.events.length.toString()),
                      _InfoLine('Active', activeEvent?.name ?? 'None'),
                      _InfoLine('Export', exportStatus),
                    ],
                  ),
                ),
                ...
                Expanded(
                  child: _InfoCard(
                    ...
                    accent: window.accent,
                    children: [
                      _ActionLine(label: 'Bookings CSV', onTap: _importBookingsCsv),
                      _ActionLine(label: 'Tickets CSV', onTap: _importTicketsCsv),
                      _ActionLine(label: 'Members CSV', onTap: _importMembersCsv),
                      _ActionLine(label: 'Schedule CSV', onTap: _importScheduleCsv),
                      _ActionLine(label: 'Game Modes CSV', onTap: _importGameModesCsv),
                      _ActionLine(label: 'Field Map Image', onTap: _importFieldMap),
                    ],
                  ),
                ),
              ],
                    ...
                    child: ListView(
                      children: [
                        ...
                        _InfoCard(
                          ...
                          accent: window.accent,
                          children: [
                            _InfoLine('Booked Persons', _eventBookedPersons(event).toString()),
                            _InfoLine('Ticket Value', '¥ ${_formatMoney(_eventTicketValue(event))}'),
                            _InfoLine('Sales Value', '¥ ${_formatMoney(_eventSalesValue(event))}'),
                            _InfoLine('Rental Gun Sets', _eventRentalCount(event).toString()),
                            _InfoLine('Pickup Bookings', _pickupGroups(event).length.toString()),
                            _InfoLine('Training Requests', _trainingGroups(event).length.toString()),
                          ],
                        ),
                      ],
                    ),
                                ...
                                Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      _summaryLine('Persons', _groupPersonCount(selectedGroup).toString()),
                                    ],
                                  ),
                                ),
                                const SizedBox(height: 12),
            ...
            children: [
              Expanded(
                ...
                child: _InfoCard(
                  ...
                  accent: window.accent,
                  children: [
                    _InfoLine('Field Map', event.fieldMapBase64 == null ? 'NOT LOADED' : 'LOADED'),
                    _InfoLine('Notes', event.notes.isEmpty ? 'NONE' : 'SEE EVENT INFO'),
                    _InfoLine('Console', propControlStatus),
                    const _InfoLine('Wi-Fi', 'JOIN PROP NETWORK FIRST'),
                    TextField(
...
class _PropWebView extends StatefulWidget {
  final String url;
  ...
  @override
  State<_PropWebView> createState() => _PropWebViewState();
}

class _PropWebViewState extends State<_PropWebView> {
...
class _KeyboardSafeArea extends StatelessWidget {
  final Widget child;

  const _KeyboardSafeArea({
    required this.child,
  });
...
class _AOJDesktopBackground extends StatelessWidget {
  const _AOJDesktopBackground();
...
class _AOJDesktopIcon extends StatelessWidget {
  final IconData icon;
  final Color accent;

  const _AOJDesktopIcon({
    required this.icon,
    required this.accent,
  });
